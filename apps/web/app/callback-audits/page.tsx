...
export default async function CallbackAuditsPage() {
  const audits = await listCallbackAudits(80);
    ...
    <section className="mx-auto max-w-[1400px]">
      <div className="mb-6 flex flex-col gap-4 border-b border-white/8 pb-5 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-semibold tracking-[-0.04em] text-champagne sm:text-4xl">
              回调验签与接收审计
          </h1>
          <p className="mt-3 max-w-3xl text-sm leading-7 text-white/45">
              这里展示最近的回调事件，包括验签失败、任务未命中、成功写入三种结果。
          </p>
        </div>
          <Link
            href="/jobs"
            className="inline-flex w-full items-center justify-center rounded-full border border-[#d6b25e]/12 bg-white/[0.04] px-5 py-3 text-sm font-medium text-champagne transition hover:bg-white/[0.08] sm:w-auto"
          >
            返回任务列表
          </Link>
      </div>
      ...
      <div className="grid gap-4">
          {audits.data.length ? (
            audits.data.map((audit) => (
              <article
                key={audit.id}
                className="rounded-[1.5rem] border border-white/8 bg-white/[0.035] p-5 backdrop-blur-xl sm:rounded-[1.75rem] sm:p-6"
              >
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                      <span
                        className={`rounded-full px-3 py-1 text-[11px] font-semibold ${
                          tone[audit.outcome] ?? "bg-zinc-100 text-zinc-700"
                        }`}
                      >
                        {audit.outcome}
                      </span>
                ...
                <details className="mt-4 rounded-[1rem] border border-[#d6b25e]/10 bg-black/18 p-4">
                  <summary className="cursor-pointer list-none text-xs font-semibold uppercase tracking-[0.18em] text-[#b89b54]">
                    Raw Payload
                  </summary>
