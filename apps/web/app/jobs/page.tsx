...
export default async function JobsPage({ searchParams }: PageProps) {
  ...
  const detailResult = selectedId ? await getBatchJob(selectedId) : { data: null, error: null };
    ...
    <section className="mx-auto max-w-[1800px]">
      <div className="mb-6 flex flex-col gap-4 border-b border-white/8 pb-5 xl:flex-row xl:items-end xl:justify-between">
        <div>
          ...
          <p className="mt-3 max-w-3xl text-sm leading-7 text-white/45">
            固定主框架，只切换局部内容。左侧看批次，右侧看当前批次和单项结果，不再来回跳页面。
          </p>
        </div>
        <div className="flex flex-wrap gap-2 text-xs">
          <span className="rounded-full border border-white/8 bg-white/[0.03] px-3 py-2 text-white/58">
            批次 {totalJobs}
          </span>
      ...
      {result.error ? (
        <div className="mb-6 rounded-[1.5rem] border border-amber-500/20 bg-amber-500/8 p-5">
          ...
          <p className="mt-3 text-sm leading-7 text-amber-100/80">
            当前无法连接到任务 API。通常是 `http://localhost:3001` 没启动，或者 API 启动异常。
          </p>
      ...
      {detailResult.error ? (
        <div className="mb-6 rounded-[1.5rem] border border-amber-500/20 bg-amber-500/8 p-5">
          ...
          <p className="mt-3 text-sm leading-7 text-amber-100/80">
            左侧批次列表已经拿到，但右侧当前批次详情读取失败。你仍然可以切换批次继续排查。
          </p>
